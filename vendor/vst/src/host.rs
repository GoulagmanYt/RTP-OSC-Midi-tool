...
pub trait Host {
    ...
    /// Return the active sample rate in Hz.
    fn get_sample_rate(&self) -> isize {
        0
    }
    /// Return the host input latency in samples.
    fn get_input_latency(&self) -> isize {
        0
    }
    /// Return the host output latency in samples.
    fn get_output_latency(&self) -> isize {
        0
    }
    /// Return the thread/process level of the current callback.
    fn get_current_process_level(&self) -> isize {
        api::ProcessLevel::Unknown as isize
    }
    /// Return the host automation read/write state.
    fn get_automation_state(&self) -> isize {
        0
    }
    /// Handle a plug-in request to rebuild its I/O configuration.
    fn io_changed(&self) -> bool {
        false
    }
    /// Report whether the host implements a named VST2 capability.
    fn can_do(&self, _capability: &str) -> Supported {
        Supported::No
    }
    ...
    /// Ask the host to resize the native editor client area.
...
pub struct PluginInstance {
    ...
    lib: Arc<Library>,
    info: Info,
    is_editor_active: Arc<AtomicBool>,
    has_editor: bool,
    processing: bool,
    resumed: bool,
    host_dropper: unsafe fn(isize),
}
...
impl<T: Host> PluginLoader<T> {
        ...
        unsafe {
            ...
            Ok(PluginLoader {
                main:
                    ...
                    match lib
                        .get::<PluginMain>(b"VSTPluginMain")
                        // Several otherwise valid legacy Windows VST2 plug-ins
                        // export the pre-2.4 entry-point name only. Hosts in the
                        // field still support it, so keep this compatibility
                        // fallback isolated to the loader.
                        .or_else(|_| lib.get::<PluginMain>(b"main"))
                    {
                        Ok(symbol) => *symbol,
                        Err(_) => return Err(PluginLoadError::NotAPlugin),
                    }
                ,
                lib: Arc::new(lib),
                host,
    ...
    unsafe fn call_main(&mut self) -> *mut AEffect {
        let _load_guard = LOAD_MUTEX.lock().unwrap_or_else(|error| error.into_inner());
        let context = Box::into_raw(Box::new(Arc::clone(&self.host))) as *mut c_void;
        LOAD_POINTER.store(context, Ordering::Release);
        let effect = (self.main)(callback_wrapper::<T>);
        let stored = LOAD_POINTER.swap(ptr::null_mut(), Ordering::AcqRel);
        if !stored.is_null() {
            drop(Box::from_raw(stored as *mut Arc<Mutex<T>>));
        }
        effect
    }
    ...
    pub fn instance(&mut self) -> Result<PluginInstance, PluginLoadError> {
        ...
        unsafe {
            let raw = &*effect;
            let flags = PluginFlags::from_bits_truncate(raw.flags);
            if raw.magic != VST_MAGIC
                || (raw.dispatcher as *const ()).is_null()
                || (raw.setParameter as *const ()).is_null()
                || (raw.getParameter as *const ()).is_null()
                || (raw.processReplacing as *const ()).is_null()
                || !flags.contains(PluginFlags::CAN_REPLACING)
                || raw.numPrograms < 0
                || raw.numParams < 0
                || raw.numInputs < 0
                || raw.numOutputs < 0
        ...
        let instance = PluginInstance::new(effect, Arc::clone(&self.lib), drop_host::<T>);
...
impl PluginInstance {
    fn new(effect: *mut AEffect, lib: Arc<Library>, host_dropper: unsafe fn(isize)) -> PluginInstance {
        ...
        let mut plug = PluginInstance {
            ...
            is_editor_active: Arc::new(AtomicBool::new(false)),
            has_editor: false,
            processing: false,
            resumed: false,
            host_dropper,
        ...
        unsafe {
            ...
            plug.has_editor = flags.contains(PluginFlags::HAS_EDITOR);
    ...
    /// Enumerate one class from a VST2 Shell plug-in. Repeated calls return
    /// successive `(unique_id, name)` pairs until the shell returns zero.
    pub fn shell_next_plugin(&self) -> Option<(i32, String)> {
        let mut name = [0i8; MAX_PRODUCT_STR_LEN];
        let id = self.dispatch(plugin::OpCode::ShellGetNextPlugin, 0, 0, name.as_mut_ptr().cast(), 0.0) as i32;
        if id == 0 {
            return None;
        }
        let bytes = name
...
trait Dispatch {
    ...
    fn write_string(&self, opcode: plugin::OpCode, index: i32, value: isize, string: &str, opt: f32) -> isize {
        ...
        self.dispatch(opcode, index, value, string.as_ptr() as *mut c_void, opt)
    }
...
impl Plugin for PluginInstance {
    ...
    fn can_do(&self, can_do: plugin::CanDo) -> Supported {
        ...
        Supported::from(self.write_string(plugin::OpCode::CanDo, 0, 0, &s, 0.0)).unwrap_or(Supported::Maybe)
    }
    ...
    fn get_input_info(&self, input: i32) -> ChannelInfo {
        let mut props: api::ChannelProperties = unsafe { std::mem::zeroed() };
        let ptr = &mut props as *mut _ as *mut c_void;
    ...
    fn get_editor(&mut self) -> Option<Box<dyn Editor>> {
        if !self.has_editor || self.is_editor_active.swap(true, Ordering::AcqRel) {
        ...
        Some(Box::new(EditorInstance {
            ...
            is_open: false,
            active: Arc::clone(&self.is_editor_active),
        }))
    }
}
...
static LOAD_POINTER: AtomicPtr<c_void> = AtomicPtr::new(ptr::null_mut());
static LOAD_MUTEX: Mutex<()> = Mutex::new(());

unsafe fn drop_host<T: Host>(raw: isize) {
    drop(Box::from_raw(raw as *mut Arc<Mutex<T>>));
}
...
extern "C" fn callback_wrapper<T: Host>(
    ...
    std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| unsafe {
        ...
        if !effect.is_null() && (*effect).reserved1 != 0 {
            ...
            let host = &mut *host.lock().unwrap_or_else(|error| error.into_inner());
        ...
        // dereferenced
            ...
            let host = LOAD_POINTER.load(Ordering::Acquire) as *const Arc<Mutex<T>>;
            if host.is_null() {
                return 0;
            }
            let host = &*host;
