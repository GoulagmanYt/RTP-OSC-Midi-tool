...
fn copy_string(dst: *mut c_void, src: &str, max: usize) -> isize {
    ...
    unsafe {
        ...
        memset(dst, 0, max);
        memcpy(dst, src.as_ptr() as *const c_void, min(max - 1, src.len()));
    }
...
pub fn host_dispatch(
    ...
    match opcode {
        ...
        Ok(OpCode::UpdateDisplay) => host.update_display(),
        Ok(OpCode::SizeWindow) => return isize::from(host.size_window(index, value as i32)),
        Ok(OpCode::IOChanged) => return isize::from(host.io_changed()),
        ...
        Ok(OpCode::CanDo) => {
            ...
            let capability = read_string(ptr);
            info!("Plugin is asking if host can: {}.", capability);
            return host.can_do(&capability).into();
        }
        ...
        Ok(OpCode::GetSampleRate) => return host.get_sample_rate(),
        Ok(OpCode::GetInputLatency) => return host.get_input_latency(),
        Ok(OpCode::GetOutputLatency) => return host.get_output_latency(),
        Ok(OpCode::GetCurrentProcessLevel) => return host.get_current_process_level(),
