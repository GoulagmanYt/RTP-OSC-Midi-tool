...
impl ControlPort {
    ...
    pub async fn start(
        ...
        buf: &mut [u8; MAX_CONTROL_PACKET_SIZE],
    ) {
        ...
        if let Err(e) = recv {
            event!(
                Level::ERROR,
                "Failed to receive data on control port: {}",
                e
            );
            return;
        }
        ...
        match packet {
            ControlPacket::Invitation { body, name } => {
                self.handle_invitation(body, name, invite_handler, ctx, src)
                    .await;
            }
            ...
            _ => {
                event!(
                    Level::WARN,
                    packet = std::format!("{:?}", packet),
                    "Control: Unhandled control packet"
                );
            }
        }
    }
    ...
    async fn handle_invitation(
        ...
        if accept {
            ...
            let rejection_packet =
                ControlPacket::new_rejection_as_bytes(invitation.initiator_token, self.ssrc);
    ...
    async fn remove_invitation(
        &self,
        invitation_response: &SessionInitiationPacketBody,
        ctx: &RtpMidiSession,
        src: SocketAddr,
    ) -> Option<PendingInvitation> {
        event!(
            Level::DEBUG,
            "Removing invitation for SSRC {} at {}",
            invitation_response.sender_ssrc,
            src
        );
    ...
    async fn handle_acceptance(
        ...
        if inv.is_none() {
            event!(
                Level::WARN,
                "Received Acknowledgment but no matching invitation found"
            );
            return;
        }
        ...
        ctx.midi_port
            .send_invitation(&response_packet, midi_addr)
            .await;
    }
}
