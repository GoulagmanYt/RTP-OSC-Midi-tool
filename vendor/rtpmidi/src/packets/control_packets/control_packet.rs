...
use crate::packets::{
    control_packets::session_initiation_packet::SessionInitiationPacketBody,
...
#[derive(
    TryFromBytes, Unaligned, KnownLayout, Immutable, Debug, Default, IntoBytes, Clone, Copy,
)]
#[repr(u8)]
#[allow(dead_code)]
...
impl<'a> ControlPacket<'a> {
    ...
    pub fn new_invitation_as_bytes(initiator_token: U32, ssrc: U32, name: &CStr) -> Bytes {
        ...
        let mut packet = BytesMut::with_capacity(
            header.len() + command.len() + body.as_bytes().len() + name_bytes.len(),
        );
    ...
    pub fn new_rejection_as_bytes(initiator_token: U32, ssrc: U32) -> Bytes {
        ...
        let mut packet =
            BytesMut::with_capacity(header.len() + command.len() + body.as_bytes().len());
        packet.extend_from_slice(&header);
        packet.extend_from_slice(command);
        packet.extend_from_slice(body.as_bytes());
        packet.freeze()
