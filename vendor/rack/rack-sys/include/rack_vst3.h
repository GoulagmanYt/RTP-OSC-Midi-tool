...
// Get preferred editor size in pixels
// Returns 0 on success, negative error code on failure
int rack_vst3_plugin_editor_size(RackVST3Plugin* plugin, float* width, float* height);

// Create a GUI object for the plugin's editor
// Returns NULL if the plugin has no editor or on failure
RackVST3Gui* rack_vst3_plugin_create_gui(RackVST3Plugin* plugin);

// Destroy GUI object and detach it if still attached
void rack_vst3_gui_free(RackVST3Gui* gui);

// Attach GUI to a parent HWND
// parent must be a valid HWND on Windows
// Returns 0 on success, negative error code on failure
int rack_vst3_gui_attach(RackVST3Gui* gui, void* parent);

// Detach GUI from its parent HWND
// Returns 0 on success, negative error code on failure
int rack_vst3_gui_detach(RackVST3Gui* gui);

// Get current GUI size in pixels
// Returns 0 on success, negative error code on failure
