...
pub struct Vst3PluginGui {
    inner: NonNull<ffi::RackVST3Gui>,
    _not_sync: PhantomData<*const ()>,
}
...
unsafe impl Send for Vst3PluginGui {}
...
impl Vst3Plugin {
    ...
    pub fn editor_size(&self) -> Result<(f32, f32)> {
        unsafe {
            let mut width = 0.0f32;
            let mut height = 0.0f32;
            let result =
                ffi::rack_vst3_plugin_editor_size(self.inner.as_ptr(), &mut width, &mut height);
    ...
    pub fn create_gui(&mut self) -> Result<Vst3PluginGui> {
        unsafe {
...
impl Vst3PluginGui {
    pub fn attach(&mut self, parent: *mut c_void) -> Result<()> {
        unsafe {
            let result = ffi::rack_vst3_gui_attach(self.inner.as_ptr(), parent);
            if result != ffi::RACK_VST3_OK {
                return Err(map_error(result));
            }
            Ok(())
    ...
    pub fn detach(&mut self) -> Result<()> {
        unsafe {
            let result = ffi::rack_vst3_gui_detach(self.inner.as_ptr());
            if result != ffi::RACK_VST3_OK {
                return Err(map_error(result));
            }
            Ok(())
    ...
    pub fn size(&self) -> Result<(f32, f32)> {
        unsafe {
            let mut width = 0.0f32;
            let mut height = 0.0f32;
            let result = ffi::rack_vst3_gui_get_size(self.inner.as_ptr(), &mut width, &mut height);
            if result != ffi::RACK_VST3_OK {
                return Err(map_error(result));
            }
            Ok((width, height))
...
impl Drop for Vst3PluginGui {
    fn drop(&mut self) {
        unsafe {
            ffi::rack_vst3_gui_free(self.inner.as_ptr());
        }
    }
}
...
impl PluginInstance for Vst3Plugin {
    ...
    fn process(
        ...
        // Defense-in-depth: output buses must exist, but instrument plugins can
        // validly expose zero audio inputs and rely solely on MIDI.
        if outputs.is_empty() {
